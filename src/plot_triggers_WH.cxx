...
int main(){ 
  ...
  TString folder("/home/users/rheller/wh_babies/babies_v31_1_2019_08_13/");
  ...
  TChain c_ht("t");
  c_ht.Add(folder+"slim_data_2016*.root");
  // if(!jet450) {c_ht.Add(folder+"/merged_database_ne1nj3_httrig_w_leps/*.root");
  //   c_ht.Add(folder+"/merged_database_nm1nj3_httrig_w_leps/*.root");}
  // else{c_ht.Add(folder+"/merged_jet_trig/mergedbaby_JetHT__nfiles_3102_nleps1_njetsge3_pass.root");}

  // TChain c_ht_nolep("tree");
  // c_ht_nolep.Add(folder+"/skim_httrig_w_leps/*.root");
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////
  // MET100: Muon
  TString basecuts= "(HLT_SingleEl==1||HLT_SingleMu==1)&&ngoodjets==2&&ngoodleps==1";
  //vector<TString> baseline({basecuts+"&&nmus==1&&met>200", basecuts+"&&nels==1&&met>200"});

  //This is the denominator selection
  vector<TString> baseline({basecuts}); //potentially add more variations.

  //This is the additional selection for numerator.
  vector<TString>trigs({"HLT_MET_MHT==1"}); //potentially add more variations.

  vector<TString> title({"Single lepton triggers, N_{lep}=1, N_{j}=2"});
  //if(jet450) title ={"Jet450, N_{#mu}=1, N_{j}#geq3, S_{T}>500", "Jet450, N_{e}=1, N_{j}#geq3, S_{T}>500"};
  vector<TString> ytitle({"MET triggers"});
  vector<TString> variable({"pfmet"}); //x-axis for plot (can add others to list)
  vector<TString> nminus1({"1"}); //additional selection corresponding to each x-axis variable in list.
  // vector<TString> title_nminus1({"&&met>200", "&&met>200","&&met>200","&&met>200","&&met>200","&&met>200"});
  vector<TString> lepton({"el+mu","Muon","Electron"});
  vector<TString> varname({"MET",});
  vector<float> v_nbins_factor({25});
  vector<float> v_minx({0});
  vector<float> v_maxx({425});
  vector<float> v_minfit({100});
  vector<TString> runrange({""/*,"&&run<280919","&&run>=280919"*/});

  vector<vector<TString> > effs;
  vector<TString> metcuts({"pfmet>0&&pfmet<=50", "pfmet>50&&pfmet<=100", "pfmet>100&&pfmet<=150", "pfmet>150&&pfmet<=200", 
	"pfmet>200&&pfmet<=250", "pfmet>250&&pfmet<=300", "pfmet>300", "pfmet>200"});
  /*
  ...
  */
  ...
  for(size_t ivar=0; ivar<variable.size(); ivar++){
    for(size_t ilep=0; ilep<baseline.size(); ilep++){
      for(size_t irun=0;irun<runrange.size();irun++){
	//if( (ilep==0 && ivar==1) || (ilep==1 &&ivar==0)) continue;
	minx=static_cast<float>(v_minx[ivar]);
	maxx=static_cast<float>(v_maxx[ivar]);
	nbins=static_cast<int>((maxx-minx)/v_nbins_factor[ivar]);
	PlotTurnOn(&c_ht,variable[ivar],nbins,minx,maxx, varname[ivar], baseline[ilep]+runrange[irun], trigs[ilep], title[ilep], 
		   ytitle[ilep], v_minfit[ivar]);
    
	// PlotTurnOn(&c_ht,variable[ivar],nbins,minx,maxx, varname[ivar], baseline[ilep]+runrange[irun]+"&&met>200", trigs[ilep], title[ilep], 
	// 	   ytitle[ilep], v_minfit[ivar]);

	// PlotTurnOn(&c_ht,variable[ivar],nbins,minx,maxx, varname[ivar], baseline[ilep]+runrange[irun]+"&&met<=200", trigs[ilep], title[ilep], 
	// 	   ytitle[ilep], v_minfit[ivar]);

	PlotTurnOn(&c_ht,variable[ivar],nbins,minx,maxx, varname[ivar], baseline[ilep]+runrange[irun],"HLT_MET_MHT==1", title[ilep], 
		   "MET triggers", v_minfit[ivar]);
    
	// PlotTurnOn(&c_ht,variable[ivar],nbins,minx,maxx, varname[ivar], baseline[ilep]+runrange[irun]+"&&met>200", "trig_met", title[ilep], 
	// 	   "MET triggers", v_minfit[ivar]);
	...
	//	PlotTurnOn(&c_ht,variable[ivar],nbins,minx,maxx, varname[ivar], baseline[ilep]+runrange[irun], "trig_lep", title[ilep], 
	//	   "Pure lepton triggers", v_minfit[ivar]);
	//	PlotTurnOn(&c_ht,variable[ivar],nbins,minx,maxx, varname[ivar], baseline[ilep]+runrange[irun], "trig_vvvl", title[ilep], 
	//	   "VVVL triggers", v_minfit[ivar]);

      }
    } // Loop over muon, electron
  }
    //  minx = 10; maxx = 140; nbins = static_cast<int>((maxx-minx)/5);
    //PlotTurnOn(&c_ht_nolep,variable[ilep],nbins,minx,maxx, lepton[ilep], baseline[ilep], trigs[ilep], title[ilep], 
    //	     ytitle[ilep], -30);
    // cout<<endl<<" ==== MET100 efficiency for "<<ytitle[ilep]<<endl<<endl;
    // minx = 0; maxx = 425; nbins = static_cast<int>((maxx-minx)/25);
    // PlotTurnOn(&c_ht,"met",nbins,minx,maxx, "E_{T}^{miss}", baseline[ilep], "pass", title[ilep], 
...
TString PlotTurnOn(TChain *data, TString var, int nbins, double minx, double maxx, TString xtitle, 
  ...
  TString lumi = "35.9 fb^{-1}";// "17.1 fb^{-1}";
